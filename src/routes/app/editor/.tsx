  ...
  const generateClips = async () => {
    ...
    try {
      ...
      if (data.errors && data.errors.length > 0) {
        const warnMsgs = data.errors.map(
          (e: any) => `Clip ${e.markerIndex + 1} ${e.variant}: ${e.error}`
        );
        setGenerateWarnings(warnMsgs);
      }
        ...
        <div className="flex flex-col items-center py-16">
          <div className="rounded-md border border-kill-red/30 bg-kill-red/10 px-6 py-8 text-center max-w-md">
            ...
            <p className="mt-2 text-gray-2">
              This video doesn't exist or may have been removed. Please upload a new video to get started.
            </p>
            <Link
              to="/app/upload"
          ...
          <div className="mt-8">
            ...
            <div className="mt-6">
              ...
              {generateWarnings.length > 0 && (
                <div className="mb-4 rounded-sm border border-headshot-yellow/30 bg-headshot-yellow/10 px-4 py-3 text-sm text-headshot-yellow">
                  <p className="font-semibold mb-1">Some clips had issues:</p>
              ...
              {generatedClips ? (
                <div className="rounded-md border border-victory-green/30 bg-victory-green/5 p-6 text-center">
                  <p className="font-subheading text-xl font-semibold text-victory-green">
                    {generatedClips.generatedCount ?? generatedClips.clips.length} clip{(generatedClips.generatedCount ?? generatedClips.clips.length) !== 1 ? "s" : ""} generated
                    {generatedClips.errors ? ` (${generatedClips.errors.length} variant${generatedClips.errors.length !== 1 ? "s" : ""} failed)` : ""}!
                  </p>
                  {generatedClips.errors && (
                    <p className="mt-1 text-sm text-gray-2">
                      Some variants couldn't be generated — see warnings above. The successful clips are available below.
                    </p>
                  )}
                  <Link
                    to="/app/clips/$id"
